...
test.beforeAll(async ({browser})=>{
   ...
   //get the cookies in json file using storage state options
   await context.storageState({path: 'EcomState.json'});

   //set the storge seesion
   webContext = await browser.newContext({storageState : 'EcomState.json'});
});

test('@API Playwright client web Test 1' , async () =>
{
...
test('Playwright client web Test 2' , async()=>{
